...
public class FisherManager extends AbstractManager {
    ...
    public float getBoostOutputMod(ItemStack item) {
        ...
        return validBoost(item) ? boostMap.get(makeNBTComparable(item)).getMiddle() : 1.0F;
    }
    ...
    public void refresh(RecipeManager recipeManager) {
        ...
        clear();
        for (var holder : recipeManager.getAllRecipesFor(FISHER_BOOST.get())) {
            addBoost(holder.value());
        }
    }
    // endregion
}
